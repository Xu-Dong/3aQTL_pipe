...
# usage: python extract_3UTR_location.py --dapars_res /xxx/xxx/Dapars2_geuv_PDUIs.all_chromosomes.txt --output /xxx/xxx/ouput/3utr_location.txt
'''
import argparse
...
print >>fho,"Gene\tChr\tStart\tEnd"
for gene in gene_order:
    if gene in gene2loc:
        print >>fho,"%s\t%s\t%s\t%s" % (gene,gene2loc[gene][0],gene2loc[gene][1],gene2loc[gene][2])
fho.close()
