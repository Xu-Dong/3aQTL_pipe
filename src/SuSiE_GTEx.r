# Expect `genotype` and `phenotype` variables from command input
# eg,
genotype = '3aQTL.vcf'
phenotype = 'expr.phen'
...
# Run SuSiE
res = susieR::susie(X, y[,2], L=10, scaled_prior_variance=0.2)
# Visualize result
...
get_susie_output = function(unit, res, pip_cutoff = 0.1) {
        ...
        out[which(out[,3] >= 0.1 | !is.na(out[,4])), ]
    }
